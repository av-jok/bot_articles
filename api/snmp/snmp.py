    ...
    def get_ifwalk(self) -> dict:
        ...
        try:
            ...
            error_indication, error_status, error_index, var_bind_table = cmd_gen.nextCmd(
                ...
                cmdgen.UdpTransportTarget((self.ip, self.port)), *oids_form)

            if error_indication:
                raise BaseException(f"errorIndication: {error_indication}")
            if error_status:
                raise BaseException(f"errorStatus: {error_status.prettyPrint(), error_index and var_bind_table[-1][int(error_index) - 1] or '?'}")
